import { useState } from "react";
import { Button, Card, CardText, CardTitle, Col, Row } from "reactstrap";
import { AiOutlineCheck } from "react-icons/ai";

const NotificationCart = () => {
    const [selected, setSelected] = useState(false);
    const data = [
        {
            "content1": "Assignment",
            "header1": "Upload Final Edits",
            "content2_1": "Album: Summer 2025",
            "content2_2": "Pending Photos: 24",
            "content2_3": "Reviewer: Anita",
            "status": "In Process"
        },
        {
            "content1": "Assignment",
            "header1": "Retouch Selected Shots",
            "content2_1": "Album: Autumn 2024",
            "content2_2": "Pending Photos: 12",
            "content2_3": "Reviewer: David",
            "status": "In Process"
        },
        {
            "content1": "Assignment",
            "header1": "Organize RAW Files",
            "content2_1": "Album: Nature Trails",
            "content2_2": "Pending Photos: 45",
            "content2_3": "Reviewer: Priya",
            "status": "In Process"
        },
        {
            "content1": "Assignment",
            "header1": "Finalize Portfolio",
            "content2_1": "Album: Urban Life",
            "content2_2": "Pending Photos: 8",
            "content2_3": "Reviewer: Kumar",
            "status": "In Process"
        },
        {
            "content1": "Assignment",
            "header1": "Upload Event Highlights",
            "content2_1": "Album: Corporate Meetup",
            "content2_2": "Pending Photos: 30",
            "content2_3": "Reviewer: Sneha",
            "status": "In Process"
        },
        {
            "content1": "Assignment",
            "header1": "Edit Drone Footage",
            "content2_1": "Album: Aerial Views",
            "content2_2": "Pending Photos: 18",
            "content2_3": "Reviewer: Raj",
            "status": "In Process"
        },
        {
            "content1": "Assignment",
            "header1": "Crop Banner Images",
            "content2_1": "Album: Product Launch",
            "content2_2": "Pending Photos: 22",
            "content2_3": "Reviewer: Meera",
            "status": "In Process"
        },
        {
            "content1": "Assignment",
            "header1": "Adjust Lighting Levels",
            "content2_1": "Album: Wedding Ceremony",
            "content2_2": "Pending Photos: 35",
            "content2_3": "Reviewer: Vikram",
            "status": "In Process"
        },
        {
            "content1": "Assignment",
            "header1": "Export for Printing",
            "content2_1": "Album: Portrait Series",
            "content2_2": "Pending Photos: 10",
            "content2_3": "Reviewer: Leena",
            "status": "In Process"
        },
        {
            "content1": "Assignment",
            "header1": "Rename Project Files",
            "content2_1": "Album: Documentary Shots",
            "content2_2": "Pending Photos: 50",
            "content2_3": "Reviewer: Arjun",
            "status": "In Process"
        }
    ]


    return (
        <>
            <div className="flex justify-center flex-wrap gap-[20px]" onClick={() => {
                setSelected((state) => !state)
            }}>
                {
                    data.map((item, index) => {
                        return (
                            <>
                                <div key={index} className={`shadow-2xl w-[380px] mx-2 h-[160px] rounded-xl flex  bg-[#FFFCFB] `} style={{ boxShadow: "1px 1px 8px 4px #a1c6ea" }}>
                                    <div className=" w-5/12">
                                        <img className=" w-[150px] h-[160px] object-cover rounded-l-xl" src="data:image/jpeg;base64," alt="image" />
                                    </div>
                                    <div className={`relative pl-3 w-7/12  ${selected ? `border-b-[3px] border-t-[3px] border-r-[3px] border-green-600 w-[220px] rounded-tr-xl rounded-br-xl` : ""}`}>
                                        {selected && <div className="absolute right-[-6px] bottom-[138px]">
                                            <AiOutlineCheck size={30} className=" bg-green-600 rounded-full text-white p-1 " />
                                        </div>}
                                        <div className=" flex flex-col gap-1 items-start">
                                            <p className="  font-medium text-[12px] mb-0 text-[#7a7979]">{item?.content1}</p>
                                            <p className=" font-bold text-[16px] mb-0 text-[#1d4f83]">{item?.header1}</p>
                                            <p className="  font-medium text-[12px] mb-0 text-[#bbb3ab]">{item.content2_1}</p>
                                            <p className="  font-medium text-[12px] mb-0 text-[#bbb3ab]">{item?.content2_2}</p>
                                            <p className="  font-medium text-[12px] mb-0 text-[#bbb3ab]">{item?.content2_3}</p>
                                        </div>
                                        <div className=" flex justify-end mx-3 mt-3">
                                            <p className=" px-2 w-fit bg-[#FFB4B4] text-[#E14434] font-medium rounded-xl text-[12px]">In Process</p>
                                        </div>
                                    </div>
                                    <div>
                                    </div>
                                </div>
                            </>
                        )
                    })
                }
            </div>
        </>
    )
}

export default NotificationCart;